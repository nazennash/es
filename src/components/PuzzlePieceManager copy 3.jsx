              ...
              <div 
                ...
                ref={(el) => {
                  if (el && (gridDimensions.width !== el.offsetWidth || gridDimensions.height !== el.offsetHeight)) {
                    setGridDimensions({ width: el.offsetWidth, height: el.offsetHeight });
                  }
                }}
              >
                    ...
                    <div
                      ...
                      ref={(el) => {
                        if (el && (cellDimensions.width !== el.offsetWidth || cellDimensions.height !== el.offsetHeight)) {
                          setCellDimensions({ width: el.offsetWidth, height: el.offsetHeight });
                        }
                      }}
                    >
                      {pieces.map(piece => {
                        if (piece.current.x === x && piece.current.y === y) {
                          const gridWidth = gridDimensions?.width || 0;
                          const gridHeight = gridDimensions?.height || 0;
