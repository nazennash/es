...
class ThreeJSManager {
  ...
  animate = () => {
    ...
    // Update only visible pieces
    this.pieceGroup.children.forEach(piece => {
      if (this.frustum.containsPoint(piece.position)) {
        piece.visible = true;
        // Update piece animation
        if (piece.userData.isAnimating) {
          this.updatePieceAnimation(piece);
        }
      } else {
        piece.visible = false;
      }
...
const MultiplayerPuzzle3D = ({ puzzleId, gameId, isHost }) => {
  ...
  useEffect(() => {
    const handleError = (error) => {
      ...
      setUi(prev => ({
        ...prev,
        error: { type: 'error', message: 'Graphics error occurred. Please refresh.' }
      }));
    };
