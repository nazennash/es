...
const ProgressStats = ({ completed, total }) => (
...
const CompletionBadge = ({ isComplete, timeElapsed }) => {
...
const GameLayout = ({ children, header }) => (
...
const GameControls = ({ gameState, onPause, timeElapsed }) => (
  <div className="flex items-center gap-3">
    <PauseButton gameState={gameState} onPause={onPause} />
    <TimeDisplay time={timeElapsed} />
  </div>
);

const PauseButton = ({ gameState, onPause }) => (
  <motion.button
    whileHover={{ scale: 1.05 }}
    whileTap={{ scale: 0.95 }}
    onClick={onPause}
...
const ProgressIndicator = ({ progress, completedPieces, totalPieces, timeElapsed }) => (
  <motion.div 
    initial={{ opacity: 0, x: 20 }}
    animate={{ opacity: 1, x: 0 }}
    className="flex items-center gap-4"
  >
    <ProgressStats completed={completedPieces} total={totalPieces} />
...
const FloatingControls = ({ 
  ...
  <div className="absolute right-4 top-4 flex flex-col gap-2">
    {[
      { icon: <ZoomIn className="w-5 h-5" />, action: onZoomIn, tooltip: "Zoom In" },
      { icon: <ZoomOut className="w-5 h-5" />, action: onZoomOut, tooltip: "Zoom Out" },
      { icon: <Maximize2 className="w-5 h-5" />, action: onResetView, tooltip: "Reset View" },
      { icon: <RotateCcw className="w-5 h-5" />, action: onResetGame, tooltip: "Reset Puzzle" },
      { icon: <Image className="w-5 h-5" />, action: onToggleReference, tooltip: "Toggle Reference" }
    ].map((control, index) => (
      <motion.button
        key={index}
        whileHover={{ scale: 1.05 }}
...
const ReferenceImage = ({ show, image }) => {
...
const MultiplayerPanel = ({ players, scores, currentUserId }) => (
  <motion.div
    initial={{ opacity: 0, x: -20 }}
    animate={{ opacity: 1, x: 0 }}
    ...
    <div className="space-y-2">
      {Object.entries(players).map(([id, player]) => (
        <PlayerCard
          key={id}
          player={player}
          score={scores[id]?.piecesPlaced || 0}
  ...
  return (
    ...
    >
      ...
      <FloatingControls
        ...
        onToggleReference={() => setShowThumbnail(!showThumbnail)}
      />
