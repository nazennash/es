...
const MultiplayerManager = ({ gameId, isHost, user, image }) => {
  ...
  const [gameStatus, setGameStatus] = useState(GAME_STATE.WAITING);
  ...
  // Timer handling
  useEffect(() => {
    if (gameStatus === GAME_STATE.PLAYING) {
  ...
  const handlePauseGame = async () => {
    ...
    const newStatus = gameStatus === GAME_STATE.PLAYING ? GAME_STATE.PAUSED : GAME_STATE.PLAYING;
  ...
  const createPuzzlePieces = async (imageUrl) => {
    ...
    try {
      ...
      if (isHost) {
        ...
        setGameStatus(GAME_STATE.WAITING);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!sceneRef.current || !rendererRef.current || gameStatus !== GAME_STATE.PLAYING) return;
  ...
  }, [gameStatus, updatePiecePosition, totalPieces, user.uid]);
    ...
    <div className="h-screen flex flex-col bg-gray-900">
      ...
      <div className="bg-gray-800 p-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          {/* Game controls - Only visible to host before game starts */}
          {isHost && gameStatus === GAME_STATE.WAITING && (
            <button
          ...
          {/* Play/Pause - Only visible to host during game */}
          {isHost && gameStatus !== GAME_STATE.WAITING && gameStatus !== GAME_STATE.COMPLETED && (
            <button
      ...
      <div className="flex-1 relative">
        ...
        {!isHost && gameStatus === GAME_STATE.WAITING && (
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-10">
            <div className="text-white text-3xl font-bold">Waiting for host to start the game...</div>
          </div>
        )}
                ...
                <div className="space-y-2 mb-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-4">
                    <div>
                      ...
                      <p className="text-lg">{formatTime(Math.floor(winner.completionTime / 1000))}</p>
                    </div>
                    <div>
