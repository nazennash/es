...
import { PuzzlePiece3D } from '../../PuzzlePiece3D';
...
// Create a Camera Controls component
...
const MultiplayerPuzzle = ({ puzzleId, gameId, isHost}) => {
            ...
            <Suspense fallback={null}>
              ...
              {pieces.map((piece) => (
                <PuzzlePiece3D
                  ...
                  piece={piece}
                  gameState={gameState}
                  onPieceMove={handlePieceMove3D}
                  onPieceSelect={(p) => setUi(prev => ({
                    ...prev,
                    selectedPiece: prev.selectedPiece?.id === p.id ? null : p
                  }))}
                />
              ))}
              ...
              {/* Grid helper */}
              <gridHelper 
                args={[gameState.difficulty * 2, gameState.difficulty * 2]} 
                position={[0, 0, 0]}
              />
            </Suspense>
          </Canvas>
  ...
  const handlePieceMove3D = async (piece, newPosition) => {
    try {
      const updatedPieces = pieces.map(p => {
        if (p.id === piece.id) {
          const isCorrect = 
            newPosition.x === p.correct.x && 
            newPosition.y === p.correct.y && 
            p.rotation % 360 === 0;
  ...
  // Add this state for managing texture loading
  const [texturesLoaded, setTexturesLoaded] = useState(false);

  // Add a texture loading handler
  useEffect(() => {
    if (gameState.imageUrl && is3DMode) {
      const loader = new THREE.TextureLoader();
      loader.load(gameState.imageUrl, 
        () => setTexturesLoaded(true),
        undefined,
        (err) => console.error('Error loading texture:', err)
      );
    }
  }, [gameState.imageUrl, is3DMode]);
