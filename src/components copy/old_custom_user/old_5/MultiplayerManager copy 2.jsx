  ...
  const createPuzzlePieces = async (imageUrl, difficulty, scene) => {
    ...
    for (let y = 0; y < difficulty; y++) {
      for (let x = 0; x < difficulty; x++) {
        ...
        // Create box for correct position
        const boxGeometry = new THREE.PlaneGeometry(pieceWidth, pieceHeight);
        const boxMaterial = new THREE.MeshBasicMaterial({ color: 0x000000, wireframe: true });
        const box = new THREE.Mesh(boxGeometry, boxMaterial);
        box.position.set(piece.userData.correctX, piece.userData.correctY, 0);
        scene.add(box);
      }
    }
