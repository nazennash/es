...
import { 
  Camera, Check, Info, Clock, ZoomIn, ZoomOut, Maximize2, RotateCcw, 
  Image, Play, Pause, Users, Link, Copy, MessageCircle, CheckCircle2, Trophy 
} from 'lucide-react';
import { 
  database, ref, set, onValue, update, remove, 
  onDisconnect, push 
} from '../../../firebase';
...
const MultiplayerPuzzleGame = () => {
  ...
  const soundSystemRef = useRef(new SoundSystem());
  ...
  const syncPieceMovement = useCallback((piece, position, isPlaced = false) => {
    ...
    if (isPlaced) {
      const statsRef = ref(database, `games/${gameId}/players/${user.id}/stats`);
      update(statsRef, {
        piecesPlaced: (playerStats[user.id]?.piecesPlaced || 0) + 1,
  ...
  const createPuzzlePieces = useCallback(async (imageUrl) => {
    ...
    const gridSize = { x: 4, y: 3 }; // Use difficulty setting
    const pieceSize = {
    ...
    for (let y = 0; y < gridSize.y; y++) {
      for (let x = 0; x < gridSize.x; x++) {
        ...
        piece.userData.lastMovedBy = null;
        piece.userData.lastMoveTime = null;
  ...
  }, [gameId, isHost, syncPieceMovement, createPlacementGuides]);
  ...
  const initializeMultiplayerGame = async (gameId, asHost = false) => {
    setIsHost(asHost);
  ...
  const createMultiplayerGame = async () => {
    ...
    // Optional: Show a notification that the link was copied
  ...
  const startGame = async () => {
    if (isHost) {
      await updateGameState({ state: 'playing' });
    }
  ...
  const togglePause = async () => {
    ...
    await updateGameState({ state: newState });
  ...
  const sendMessage = () => {
    ...
    push(messageRef, {
      ...
      playerId: user.id,
      playerName: user.name,
      playerColor: getPlayerColor(user.id),
      timestamp: Date.now()
    });
  ...
  const syncMousePosition = useCallback((event) => {
    ...
    const cursorRef = ref(database, `games/${gameId}/cursors/${user.id}`);
    set(cursorRef, {
      position: { x, y },
      playerName: user.name,
      color: getPlayerColor(user.id),
  ...
  useEffect(() => {
    ...
    return () => unsubscribe();
  }, [gameId, image, createPuzzlePieces]);
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (event) => {
      ...
      if (intersects.length > 0) {
        ...
        // Check if piece is already claimed by another player
        if (piece.userData.lastMovedBy && 
            piece.userData.lastMovedBy !== user.id && 
    ...
    const handleMouseUp = () => {
      ...
      if (distance < 0.3) {
        ...
        if (!selectedPieceRef.current.userData.isPlaced) {
          ...
          if (gameId) {
            syncPieceMovement(selectedPieceRef.current, originalPos, true);
          }
        }
      }
  ...
  }, [gameId, syncPieceMovement, totalPieces]);
  ...
  useEffect(() => {
    return () => {
      if (gameId) {
        ...
        remove(ref(database, `games/${gameId}/players/${user.id}`));
        remove(ref(database, `games/${gameId}/cursors/${user.id}`));
        
        // If host, cleanup game if no other players
        if (isHost) {
          const gameRef = ref(database, `games/${gameId}`);
  ...
  return (
    <div className="w-full h-screen flex flex-col bg-gray-900">
        ...
        <div className="flex items-center gap-4">
          ...
          <label className="relative cursor-pointer">
            <input
              ...
              className="hidden"
              disabled={!isHost && gameState !== 'initial'}
            />
            <div className={`flex items-center gap-2 px-4 py-2 rounded-lg text-white transition-colors
              ${(!isHost && gameState !== 'initial') 
          ...
          <div className="flex items-center gap-2">
            ...
            {gameState === 'initial' && isHost && image && (
              <button
                onClick={startGame}
      ...
      <div className="flex-1 flex">
        ...
        // Add in the header controls, where we check if there's no game yet
        {!gameId && (
          <button
